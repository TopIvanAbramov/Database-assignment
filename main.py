...
try:
    cur.execute(generate_inserts.insert_room(1, 5))

    cur.execute(generate_inserts.insert_patient(101, 105, 1, 5))

    cur.execute(generate_inserts.insert_employee(1, 5, "doctor"))

    cur.execute(generate_inserts.insert_employee(5, 10, "nurse"))

    cur.execute(generate_inserts.insert_prescribe(3, 1, 5, 101, 105))

    cur.execute(generate_inserts.insert_analysis_result(5, 101, 105))

    cur.execute(generate_inserts.insert_attends(3, 1, 5, 101, 105))

    cur.execute(generate_inserts.insert_chat(3, 1, 5, 101, 105))

    cur.execute(generate_inserts.insert_inventory(3))

    cur.execute(generate_inserts.insert_treatment_plan(3, 1, 5, 101, 105))

    cur.execute(generate_inserts.insert_uses(3, 0, 3, 0, 3))
except:
